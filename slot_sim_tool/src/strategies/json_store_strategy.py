...
class JsonStoreStrategy(GameStrategy):
    ...
    def spin(self, user_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # --- Step 1: Hit or Miss ---
        # "At the beginning randomly determine win or loss"
        base_win_rate = self.config.get("base_win_rate", 0.15)
        rnd_roll = random.random()
        
        # Debug Log - Pre
        # print(f"[DEBUG] Spin {user_state.get('total_spins')} | Bet: {bet} | Roll: {rnd_roll:.4f} vs WinRate: {base_win_rate} ...")

        if rnd_roll > base_win_rate:
            print(f"[Log] Spin: Loss (Roll {rnd_roll:.4f} > {base_win_rate})")
            return self._get_outcome_from_group(0, bet) # Force Loss

        # --- Step 2: RTP & Range Check ---
        # "If win, need to check if RTP doesn't exceed 1 ... check which groups fit the range"
        ...
        # New RTP = (Current Won + Win) / (Current Wagered) <= Max RTP
        # Win <= (Max RTP * Current Wagered) - Current Won
        # Note: current_wagered already includes the current bet in engine.py
        allowed_payout = (max_rtp * current_wagered) - current_won
        
        # If we are early in sim (e.g. first spin), allowed_payout might be small or negative if we are winning
        # But usually max_rtp 1.0 allows ample room unless we are already profiting.
        # If we are already above RTP 1.0, allowed_payout will be negative -> No win allowed?
        # User said "If win ... check ...". If we can't win, maybe we fallback to loss.
        ...
        for idx in range(1, 7):
            ...
            if min_req > max_multiplier_allowed:
                # print(f"  [Debug] Group {idx} excluded. Min {min_req} > MaxAllowed {max_multiplier_allowed:.2f}")
        ...
        if not pool_indices:
            ...
            print(f"[Log] Spin: Loss (RTP Constrained). MaxMultAllowed: {max_multiplier_allowed:.2f}")
        ...
        print(f"[Log] Spin: Win! Group: {selected_idx}. Pool: {pool_indices}. MaxMultAllowed: {max_multiplier_allowed:.2f}")
