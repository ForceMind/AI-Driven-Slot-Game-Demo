...
2.  **高额投注检查 (High Roller Check - 已禁用)**：
    *   **原逻辑**：如果 `bet`（下注额）低于阈值，则自动禁用最高等级的奖池。
    *   **现状**：为了保证游戏的公平性和透明度，该功能目前已在代码中禁用。所有下注额度均有权获得所有等级的奖池。该逻辑可作为运营策略在需要时重新开启。
    为什么会有这个设计？（设计初衷）
    这个功能通常被称为 "Bet Gating"（下注门槛） 或 "Jackpot Qualification"（大奖资格），是实体赌场老虎机中非常常见的一种心理诱导机制。

    设计这个功能的初衷是基于以下几点“赌场数值策划”的经验：

    诱导提高下注 (Upselling)：
    如果玩家知道下注 10 块钱永远中不了大奖，他们就会倾向于把下注额提高到 50。这能显著提高游戏的流水（Total Wager）。
    风险控制 (Risk Management)：
    小额下注玩家通常是来“磨时间”的。如果让他们频繁中几千倍的大奖，虽然绝对金额不大，但会破坏游戏的“稀缺感”。
    大奖通常被视为对承担高风险（高下注）玩家的一种奖励。
    区分玩家层级：
    把休闲玩家（Low Rollers）和赌徒（High Rollers）的体验区分开。休闲玩家体验的是“细水长流”，赌徒体验的是“大起大落”。
